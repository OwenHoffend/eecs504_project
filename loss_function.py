...
def rpn_loss(reg, score, anchors, labels, lens, rois, device, img=None):
    ...
    #CLASS LOSS
    pos_loss = p_star*torch.log(score + 1e-15)
    neg_loss = (1-p_star)*torch.log(1-score + 1e-15)
    cls_loss = -torch.sum(pos_loss + neg_loss) / (A*N)
    ...
    if img != None:
        ...
        util.add_bbs(np_img, rois_minmax[img_ind, score[img_ind] > 0.05, :], (0, 255, 64))

        plt.imshow(np_img)
        plt.show()

        print(cls_loss.item())
        print(reg_loss.item() * lambda_)
